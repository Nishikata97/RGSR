...
from contrastive import relation_nce_loss, info_nce_loss_overall
...
class CombineGraph(Module):
    def __init__(self, opt, num_node, i_text, vocabulary, topics, item2topic):
        ...
        self.inter_encoder = GGNN(input_dim, opt.hiddenSize, batch_norm=opt.batch_norm, feat_drop=opt.dropout_inter)
        self.text_encoder = RE_GNN(input_dim, opt.hiddenSize, opt.word_dim, batch_norm=opt.batch_norm, dropout_local=opt.dropout_text)
        ...
        self.tau = opt.tau
        self.dense = nn.Parameter(torch.Tensor(self.dim, self.dim))
        self.dense2 = nn.Parameter(torch.Tensor(2*self.dim, self.dim))
    ...
    def generate_sess_emb_npos(self, alias_inputs, feat, mask):
        get = lambda index: feat[index][alias_inputs[index]]  
        seq_h = torch.stack([get(i) for i in torch.arange(len(alias_inputs)).long()])

        mask = mask.float().unsqueeze(-1)
        hs = torch.div(torch.sum(seq_h, 1), torch.sum(mask, 1))
    ...
    def generate_sess_emb_pos(self, alias_inputs, feat, mask): 
        get = lambda index: feat[index][alias_inputs[index]]  
        seq_h = torch.stack([get(i) for i in torch.arange(len(alias_inputs)).long()])
    ...
    def forward(self, alias_inputs, items, adj, adj_sg, topics, mask_item, inputs):
        ...
        inter_feat = self.inter_encoder(h_local, adj)
        text_feat = self.text_encoder(h_local, items_review, adj_sg, attn_review_mask)

        inter_sess_embedd = self.generate_sess_emb_npos(alias_inputs, inter_feat, mask_item)  
        text_sess_embedd = self.generate_sess_emb_npos(alias_inputs, text_feat, mask_item)

        batch_sess_loss_1 = info_nce_loss_overall(inter_sess_embedd, text_sess_embedd, inter_sess_embedd, self.tau)
        batch_sess_loss_2 = info_nce_loss_overall(text_sess_embedd, inter_sess_embedd, text_sess_embedd, self.tau)
        batch_sess_loss = batch_sess_loss_1 + batch_sess_loss_2


        sess_embedd = torch.cat([inter_sess_embedd, text_sess_embedd], dim=-1)
        sess_embedd = torch.matmul(sess_embedd, self.dense2)
        item_scores = torch.matmul(sess_embedd, self.node_embedding.weight[1:].transpose(1, 0))

        return item_scores, batch_sess_loss
...
def forward(model, data):
    ...
    item_scores, batch_sess_loss = model(alias_inputs, items, adj, adj_sg, topics, mask, inputs) 
    return targets, item_scores, batch_sess_loss
...
def train_test(opt, model, train_data, test_data):
    ...
    for data in tqdm(train_loader):
        ...
        targets, item_scores, batch_sess_loss = forward(model, data)
        targets = trans_to_cuda(targets).long()
        loss = model.loss_function(item_scores, targets - 1)
        loss += opt.beta * batch_sess_loss
        loss.backward()
    ...
    for data in test_loader:
        ...
        sub_20_scores = item_scores.topk(20)[1] 
        ...
        sub_10_scores = item_scores.topk(10)[1]
        ...
        sub_5_scores = item_scores.topk(5)[1]
